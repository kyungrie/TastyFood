...
<section class="bg-[#f4f4f4] grid grid-cols-1 md:grid-cols-2 px-5 py-3 md:px-[3rem] md:py-[3rem] md:gap-14">
    ...
    <div class="">
            ...
            <button class="bg-black text-white py-5 px-20 md:text-[1.5rem] mt-10 font-bold hover:scale-105 md:hover:scale-125">BACA SELENGKAPNYA</button>
        </div>
</section>
...
<section class="bg-[#ffffff] px-5 py-3 md:px-24 md:py-20">
    ...
    <div class="grid grid-cols-2 h-[150vh] gap-3 md:grid-cols-4 md:grid-rows-2">
        @foreach($index as $n)
        <x-news-card src="{{ asset('news-picture/'.$n->picture) }}"  title="{{$n->title}}">
            {{ $n->newscontent }}
        </x-news-card>
        @endforeach
    </div>
